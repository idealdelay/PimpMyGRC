...
class Block(CoreBlock, Drawable):
    ...
    def create_comment_layout(self):
        ...
        if Actions.TOGGLE_SHOW_FLOWGRAPH_COMPLEXITY.get_active() and self.key == "options":
            ...
            markups.append(
                '<span foreground="#33cc66" size="medium" font_desc="{font}">'
        ...
        if comment:
            ...
            markups.append(
                '<span foreground="{foreground}" font_desc="{font}">{comment}</span>'.format(
                    foreground='#33cc66' if self.enabled else '#557744',
                    font=Constants.BLOCK_FONT,
    ...
    def draw(self, cr):
        ...
        if self.highlighted:
            try:
                ...
                cr.set_source_rgba(0.0, 1.0, 1.0, 0.45)
                cr.fill()
                cr.restore()
            except Exception:
                cr.restore()
        ...
        # Per-state title color
        if self.state == 'enabled':
            cr.set_source_rgba(1.0, 1.0, 1.0, 1.0)
        elif self.state == 'bypassed':
            cr.set_source_rgba(1.0, 0.7, 0.28, 1.0)
        else:
