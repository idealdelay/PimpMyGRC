...
class Port(CorePort, Drawable):
    ...
    def __init__(self, parent, direction, **n):
        ...
        self._font_color = [0.0, 0.0, 0.0, 1.0]  # RGBA for solid black
    ...
    def _update_colors(self):
        ...
        # Auto-contrast: white on dark ports, black on bright
        if color:
            lum = 0.299 * color[0] + 0.587 * color[1] + 0.114 * color[2]
            if not self.parent_block.enabled:
                self._font_color = [0.6, 0.6, 0.6, 0.4]
            elif lum > 0.45:
                self._font_color = [0.0, 0.0, 0.0, 1.0]
            else:
                self._font_color = [1.0, 1.0, 1.0, 1.0]
    ...
    def draw(self, cr):
        ...
        # Outer glow (pulsing when hovering + effect enabled)
        if self.parent_block.enabled and self._area:
            try:
                gx, gy, gw, gh = self._area
                cr.save()
                if (self._hovering and effects and
                        effects.is_enabled('port_hover_glow')):
                    pulse = 0.35 + 0.25 * math.sin(time.time() * 5.0)
                    spread = 3 + 2 * math.sin(time.time() * 5.0)
                    cr.rectangle(gx - spread, gy - spread,
                                 gw + spread * 2, gh + spread * 2)
                    cr.set_source_rgba(self._bg_color[0], self._bg_color[1],
                                       self._bg_color[2], pulse)
                else:
                    cr.rectangle(gx - 3, gy - 3, gw + 6, gh + 6)
                    cr.set_source_rgba(self._bg_color[0], self._bg_color[1],
                                       self._bg_color[2], 0.35)
                cr.fill()
                cr.restore()
            except Exception:
                pass
