...
class Block(CoreBlock, Drawable):
    ...
    def create_comment_layout(self):
        ...
        if Actions.TOGGLE_SHOW_FLOWGRAPH_COMPLEXITY.get_active() and self.key == "options":
            ...
            markups.append(
                '<span foreground="#888844" size="medium" font_desc="{font}">'
        ...
        if comment:
            ...
            markups.append(
                '<span foreground="{foreground}" font_desc="{font}">{comment}</span>'.format(
                    foreground='#888844' if self.enabled else '#333318',
                    font=Constants.BLOCK_FONT,
    ...
    def draw(self, cr):
        ...
        # Glow halo when highlighted — amber
        if self.highlighted:
            try:
                cr.save()
        ...
        # Per-state title color — military/tactical
        if self.state == 'enabled':
            cr.set_source_rgba(0.8, 0.8, 0.4, 1.0)    # amber-green
        elif self.state == 'bypassed':
            cr.set_source_rgba(1.0, 0.70, 0.0, 1.0)   # amber
        else:
            cr.set_source_rgba(0.58, 0.62, 0.42, 0.64)   # dim olive
