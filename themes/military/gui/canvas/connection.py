...
class Connection(CoreConnection, Drawable):
    ...
    # ---- Military/Tactical palette (RGBA) ----
    _NEON_WIRE = (0.67, 0.67, 0.27, 1.0)       # olive-yellow
    _NEON_ARROW = (0.67, 0.67, 0.27, 1.0)      # olive-yellow
    _NEON_HIGHLIGHT = (1.00, 0.70, 0.00, 1.0)  # amber
    _NEON_DISABLED = (0.10, 0.12, 0.06, 1.0)   # dim olive
    _NEON_ERROR = (1.00, 0.27, 0.00, 1.0)      # red-orange
    # ------------------------------------------
    ...
    def draw(self, cr):
        ...
        # Force tactical colors for enabled+valid+not-highlighted
        ...
        if self.highlighted and self._line_path:
            try:
                ...
                cr.set_source_rgba(1.0, 0.70, 0.0, 0.85)
                cr.stroke()
                cr.restore()
            except Exception:
                cr.restore()
