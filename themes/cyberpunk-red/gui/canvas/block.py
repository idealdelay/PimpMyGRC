...
class Block(CoreBlock, Drawable):
    ...
    def create_comment_layout(self):
        ...
        if Actions.TOGGLE_SHOW_FLOWGRAPH_COMPLEXITY.get_active() and self.key == "options":
            ...
            markups.append(
                '<span foreground="#CC4444" size="medium" font_desc="{font}">'
        ...
        if comment:
            ...
            markups.append(
                '<span foreground="{foreground}" font_desc="{font}">{comment}</span>'.format(
                    foreground='#CC4444' if self.enabled else '#663A3A',
                    font=Constants.BLOCK_FONT,
    ...
    def draw(self, cr):
        ...
        # Glow halo when highlighted — red-orange cyberpunk
        if self.highlighted:
            try:
                cr.save()
                self._rounded_rect(cr, x - 8, y - 8, width + 16, height + 16)
                cr.set_source_rgba(1.0, 0.2, 0.0, 0.20)
                cr.fill()
                self._rounded_rect(cr, x - 4, y - 4, width + 8, height + 8)
                cr.set_source_rgba(1.0, 0.2, 0.0, 0.45)
                cr.fill()
                cr.restore()
            except Exception:
                cr.restore()
        ...
        try:
            ...
            if self.state != 'enabled':
                # Keep disabled blocks visibly "off" without sinking into the dark canvas.
                r1 = min(r1 * 0.97 + 0.03, 1.0)
                g1 = min(g1 * 0.97 + 0.03, 1.0)
                b1 = min(b1 * 0.97 + 0.03, 1.0)
            grad = cairo.LinearGradient(x, y, x, y + height)
            if self.state != 'enabled':
                grad.add_color_stop_rgba(0.0, min(r1 + 0.01, 1), min(g1 + 0.01, 1), min(b1 + 0.02, 1), 1.0)
                grad.add_color_stop_rgba(1.0, max(r1 - 0.04, 0), max(g1 - 0.04, 0), max(b1 - 0.04, 0), 1.0)
            else:
        ...
        # Per-state title color — cyberpunk red
        if self.state == 'enabled':
            cr.set_source_rgba(1.0, 0.27, 0.27, 1.0)    # bright red
        elif self.state == 'bypassed':
            cr.set_source_rgba(1.0, 0.70, 0.0, 1.0)     # gold
        else:
            cr.set_source_rgba(0.62, 0.38, 0.38, 0.64)       # deeply muted disabled red
