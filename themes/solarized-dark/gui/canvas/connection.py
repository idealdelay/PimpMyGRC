...
class Connection(CoreConnection, Drawable):
    ...
    # ---- Solarized Dark palette (RGBA) ----
    _NEON_WIRE = (0.16, 0.63, 0.60, 1.0)       # solarized cyan
    _NEON_ARROW = (0.15, 0.55, 0.82, 1.0)      # solarized blue
    _NEON_HIGHLIGHT = (0.80, 0.29, 0.09, 1.0)  # solarized orange
    _NEON_DISABLED = (0.03, 0.21, 0.26, 1.0)   # base02
    _NEON_ERROR = (0.86, 0.20, 0.18, 1.0)      # solarized red
    # ------------------------------------------
    ...
    def draw(self, cr):
        ...
        if self.highlighted and self._line_path:
            try:
                ...
                cr.set_source_rgba(0.80, 0.29, 0.09, 0.85)  # solarized orange
                cr.stroke()
                cr.restore()
            except Exception:
                cr.restore()
